...
#select and save events with polygon and location info,
...
paddd_trop = subset(paddd_whole, GeoDataTyp == "Polygon" & Location_K == "Y" & 
                    ...
                    YearPADDD >= 1990 & EnactedPro == "Enacted") #704
...
polyg_veg = polyg_trop[polyg_trop$OBJECTID %in% subset(area_df, prop_vegetation >= 0.5)$OBJECTID] #499
terra::writeVector(polyg_veg, "paddd_selected_vegetation.shp", overwrite = T)

paddd_veg = subset(paddd_trop, PADDD_ID %in% polyg_veg$PADDD_ID) #499
write.table(paddd_veg, "paddd_selected_vegetation.csv", sep = ",", row.names = F)

polyg_for = polyg_trop[polyg_trop$OBJECTID %in% subset(area_df, prop_forest >= 0.5)$OBJECTID] #451
